...
if [ -x "$CONDA_ROOT/envs/LLM/bin/uvicorn" ]; then
  PYTHONUNBUFFERED=1 "$CONDA_ROOT/envs/LLM/bin/uvicorn" main:app --host "$HOST" --port "$PORT" --log-level info --access-log >>"$LOG_FILE" 2>&1
else
  PYTHONUNBUFFERED=1 "$CONDA_ROOT/bin/conda" run -n LLM uvicorn main:app --host "$HOST" --port "$PORT" --log-level info --access-log >>"$LOG_FILE" 2>&1
fi
